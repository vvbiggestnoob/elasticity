# -*- coding: utf-8 -*-
"""
hjb_step_eik.py — HJB 方程驱动的单次 SDF 演化（拓扑优化主循环 Step B）
                  基于 hjb_step.py，新增 eikonal 正则 L_eik（|∇φ|≈1）。
...
流程：
  ...
  3. 训练 phi 拟合 t ∈ [t_current, t_current + dt] 上的 HJB 解，损失加权和
     （均不乘掩码 S）：
         ...
         L_eik     eikonal 正则（可选，λ_eik 默认 0.1）：(|∇φ|−1)² 压在内部
                   时空点上，保持 SDF 的距离函数性质、抑制演化中 |∇φ| 漂移。
                   注意 HJB 真解在空间变 V_n 下 |∇φ| 本不守恒（与 L_r、
                   L_anchor 存在固有张力），故只取小权重作正则；
                   置 lam_eik = 0 关闭
  ...
  4. 两阶段训练：Adam（Sobol 点池 + 每步 minibatch 重采，余弦退火 1e-3→1e-5）
     ...
     早停/回滚默认关闭）。
  5. 诊断（说明文档 §9）+ 出图 + 保存 weights/phi_iter1_eik.pt 与
     weights/hjb_state_eik.json（输出独立于 hjb_step.py，互不覆盖）。
...
class HJBConfig:
    ...
    # eikonal 正则权重：(|∇φ|−1)² 与 L_r 共用内部时空配点和自动微分图。
    # 小权重作正则——HJB 真解在变 V_n 下 |∇φ| 不守恒，权重过大将扭曲界面运动
    lam_eik: float = 0.001
    ...
    pool_init_corner: int = 1000   # 初始条件角点加密（4β 见方角域，与带同权重）
    w_ic_band: float = 10.0        # 初始条件边界带权重（同 sdf_init 的 w_bnd）

    # 角部硬非设计域（2026-09-14 起）：四角 corner_freeze 见方区域冻结为材料，
    # 不参与演化——V_n 在冻结区置 0（边界不动），并加角部冻结损失把 φ 锚到
    # phi_ref(t_current)（整个时间窗口），强制保留角部（桥式结构的拱脚）。
    # 0 = 关闭（默认，与原行为一致）。用于解决角部 inflow 边界 + 网络平滑
    # 导致的角部误侵蚀（见 exp01_bridge/README.md）。
    corner_freeze: float = 0.0      # 角部冻结区边长（0=关闭）
    w_corner_freeze: float = 10.0   # 角部冻结损失权重
    pool_corner_freeze: int = 4000  # Adam 角部冻结时空点池
    batch_corner_freeze: int = 512  # Adam 每步 minibatch 角部冻结点数
    lbfgs_corner_freeze: int = 1024 # L-BFGS 固定集角部冻结点数
    val_corner_freeze: int = 512    # 验证集角部冻结点数
    ...
    # Adam 停滞提前切换：过 min_step 后，window 步内 total 相对改善 < rtol
    # 则提前进入 L-BFGS（逻辑同 mech_init.py；交替循环中由循环脚本开启）
    adam_early_switch: bool = False
    adam_switch_min_step: int = 600
    ...
    early_stop_rtol: float = 1e-4     # 逐块相对下降阈值
    ...
    phi_out: str = "weights/phi_iter1_eik.pt"       # 输出：演化后 SDF（说明文档 §10）
    state_out: str = "weights/hjb_state_eik.json"   # 输出：伪时间与诊断记录
    fig_dir: str = "hjb_figures_eik"
    seed: int = 20260908
...
@dataclass
class CornerSet:
    """角部硬非设计域冻结点：四角 corner_freeze 见方区域内的时空点。

    phi0 为 phi_ref(xy, t_current)（冻结目标 = 演化前几何的角部材料形状）。
    与 InitSet 的区别：角部冻结点遍布整个时间窗口（t ∈ [t0, t_new]），
    把角部 φ 钉死在演化前形状，强制角部材料不随演化改变。
    """
    xy: torch.Tensor      # (N,2)
    t: torch.Tensor       # (N,1)，遍布 [t_current, t_new]
    phi0: torch.Tensor    # (N,1)，phi_ref(xy, t_current)

    def subset(self, idx: torch.Tensor) -> "CornerSet":
        return CornerSet(self.xy[idx], self.t[idx], self.phi0[idx])
...
class PointSets:
    ...
    init: InitSet
    corner: Optional[CornerSet] = None   # 角部硬非设计域（corner_freeze>0 时非 None）
...
def corner_mask(xy: torch.Tensor, cfg: HJBConfig) -> torch.Tensor:
    """角部硬非设计域指示函数：四角 corner_freeze 见方区域内为 1，其余为 0。

    corner_freeze <= 0 时全 0（关闭）。冻结区内 V_n 置 0（见 compute_vn），
    并由角部冻结损失把 φ 锚到 phi_ref（见 corner_freeze_loss）。
    """
    if cfg.corner_freeze <= 0.0:
        return torch.zeros(xy.shape[0], 1, dtype=cfg.dtype)
    x, y = xy[:, 0:1], xy[:, 1:2]
    cf = cfg.corner_freeze
    in_x = (x < cf) | (x > cfg.lx - cf)
    in_y = (y < cf) | (y > cfg.ly - cf)
    return (in_x & in_y).to(cfg.dtype)
...
def compute_vn(mech, phi_ref, xy: torch.Tensor, cfg: HJBConfig,
    ...
    # 角部硬非设计域：冻结区 V_n 置 0（边界不动，材料保留；锚点目标随之退化为
    # phi_ref，与角部冻结损失一致）
    if cfg.corner_freeze > 0.0:
        vn = vn * (1.0 - corner_mask(xy, cfg))
    return vn, eng, kin
...
def sample_init_corners(n: int, cfg: HJBConfig, gen: torch.Generator) -> torch.Tensor:
    """初始条件角点加密：四个角各一份（最大余数法分配），在 4β 见方的角域内
    向域内均匀采样，返回 (n, 2)。

    角域是两条边界带的交汇区，初始 SDF 在角上曲率最大（rim 拐角），光滑网络
    最容易把角抹圆；角点与边界带同权重（w_ic_band），在 build_point_sets 里
    并入 xy_band 一起锚定。
    """
    side = 4.0 * cfg.beta
    base, rem = divmod(n, 4)
    ...
    for (cx, cy, sx, sy), m in zip(corners, counts):
        if m == 0:
            continue
        u = torch.rand(m, 2, generator=gen, dtype=cfg.dtype) * side
...
def sample_corner_freeze(n: int, cfg: HJBConfig, gen: torch.Generator) -> torch.Tensor:
    """角部硬非设计域空间点：四角 corner_freeze 见方区域内均匀采样（最大余数法）。"""
    cf = cfg.corner_freeze
    base, rem = divmod(n, 4)
    counts = [base + (1 if i < rem else 0) for i in range(4)]
    # (角点 x, 角点 y, x 向域内符号, y 向域内符号)
    ...
    for (cx, cy, sx, sy), m in zip(corners, counts):
        if m == 0:
            continue
        u = torch.rand(m, 2, generator=gen, dtype=cfg.dtype) * cf
        pts.append(torch.stack([cx + sx * u[:, 0], cy + sy * u[:, 1]], dim=1))
    return torch.cat(pts, dim=0)


def build_corner_set(n: int, cfg: HJBConfig, phi_ref,
                     gen: torch.Generator) -> Optional[CornerSet]:
    """角部冻结点集：空间在四角冻结区、时间遍布整个窗口，目标 = phi_ref(t_current)。"""
    if cfg.corner_freeze <= 0.0 or n <= 0:
        return None
    xy = sample_corner_freeze(n, cfg, gen)
    t = torch.rand(n, 1, generator=gen, dtype=cfg.dtype) * cfg.dt + cfg.t_current
    with torch.no_grad():
        phi0 = eval_phi_at(phi_ref, xy, cfg.t_current)
    return CornerSet(xy=xy, t=t, phi0=phi0)
...
def build_point_sets(cfg: HJBConfig, mech, phi_ref, omega2: float, area: float,
                     ...
                     init_band_n: int, init_corner_n: int = 0,
                     corner_freeze_n: int = 0) -> PointSets:
    """构建一套点集（Adam 点池 / L-BFGS 固定集 / 验证集共用此入口）。

    角点（init_corner_n > 0 时）并入边界带 xy_band，与带同权重 w_ic_band。
    角部硬非设计域（corner_freeze_n > 0 且 cfg.corner_freeze > 0）单独成集。
    """
    ...
    if init_corner_n > 0:
        xy_band = torch.cat([xy_band, sample_init_corners(init_corner_n, cfg, gen)], dim=0)
    return PointSets(
        interior=build_interior_set(xy_i, t_i, mech, phi_ref, cfg, omega2, area),
        init=build_init_set(xy_0, t_0, xy_band, phi_ref, cfg),
        corner=build_corner_set(corner_freeze_n, cfg, phi_ref, gen),
    )


# ---------------------------------------------------------------------------
# 损失（加权和，不乘掩码 S）：L_r + L_0 (+ 可选 L_anchor, L_eik)；λ_r = λ_0 = λ_anchor = 1
# ---------------------------------------------------------------------------
...
def hjb_residual_loss(phi, pts: InteriorSet, create_graph: bool = True):
    ...
    返回 (loss, r.detach(), gnorm)：gnorm 供 eikonal 损失复用同一张自动微分图，
    避免重复求导。
    """
    ...
    return (r ** 2).mean(), r.detach(), gnorm
...
def eikonal_loss(gnorm: torch.Tensor) -> torch.Tensor:
    """(4) eikonal 正则：(|∇φ̂|−1)²，约束 SDF 保持距离函数性质。

    gnorm 由 hjb_residual_loss 一并返回（同一张自动微分图，零额外求导开销）。
    注意 HJB 真解在空间变 V_n 下 |∇φ| 不守恒，故只取小权重作正则，
    防止多步演化后 |∇φ| 漂移失控（等效界面厚度 tanh(φ/2β) 被扭曲）。
    """
    return ((gnorm - 1.0) ** 2).mean()


def corner_freeze_loss(phi, pts: CornerSet) -> torch.Tensor:
    """(5) 角部硬非设计域：φ 在角部时空点锚定到 phi_ref(t_current)（冻结为材料）。

    角部是 V_n>0 的 inflow 边界（特征线指向域内），L_b 移除后无边界条件，
    光滑 SDF 网络在相邻边侵蚀的平滑作用下守不住角帽（见 exp01_bridge/README）。
    本损失把角部 φ 在整个时间窗口钉死在演化前形状，强制保留角部材料。
    """
    pred = phi(torch.cat([pts.xy, pts.t], dim=1))
    return ((pred - pts.phi0) ** 2).mean()


def compute_total_loss(phi, data: PointSets, cfg: HJBConfig,
                       create_graph: bool = True) -> Tuple[torch.Tensor, Dict]:
    """总损失 L = λ_r·L_r + λ_0·L_0 (+ λ_a·L_anchor + λ_e·L_eik)。返回 (total, parts)。

    lam_r = lam_eik = 0 时跳过残差/梯度自动微分（廉价预测步：L_0+L_anchor 纯回归，
    无需二阶图，每步成本低数倍；真实 PDE 残差仍由诊断 residual_rms 独立评估）。
    """
    l_0 = init_loss(phi, data.init)
    total = cfg.lam_0 * l_0
    parts = {"init": float(l_0.detach())}
    if cfg.lam_r > 0.0 or cfg.lam_eik > 0.0:
        l_r, _, gnorm = hjb_residual_loss(phi, data.interior, create_graph)
        total = total + cfg.lam_r * l_r
        parts["r"] = float(l_r.detach())
        if cfg.lam_eik > 0.0:
            l_e = eikonal_loss(gnorm)
            total = total + cfg.lam_eik * l_e
            parts["eik"] = float(l_e.detach())
    if cfg.lam_anchor > 0.0:
        l_a = anchor_loss(phi, data.interior)
        total = total + cfg.lam_anchor * l_a
        parts["anchor"] = float(l_a.detach())
    if data.corner is not None and cfg.w_corner_freeze > 0.0:
        l_c = corner_freeze_loss(phi, data.corner)
        total = total + cfg.w_corner_freeze * l_c
        parts["corner"] = float(l_c.detach())
...
def train_adam(phi, phi_ref, pools: PointSets, cfg: HJBConfig,
    ...
    n_c = pools.corner.xy.shape[0] if pools.corner is not None else 0
    ...
    print(f"[Adam] 开始：{cfg.adam_steps} 步，lr {cfg.lr_init}→{cfg.lr_final}"
          ...
          + (f"+{cfg.batch_corner_freeze}(角冻)" if pools.corner is not None else ""))
    t0 = time.time()
    for step in range(cfg.adam_steps):
        data = PointSets(
            ...
            corner=(pools.corner.subset(minibatch_idx(n_c, cfg.batch_corner_freeze, gen))
                    if pools.corner is not None else None),
        )
        ...
        if step % cfg.log_every == 0 or step == cfg.adam_steps - 1:
            ...
            r_str = f"r {parts['r']:.3e} " if "r" in parts else ""
            anc = f" anchor {parts['anchor']:.3e}" if "anchor" in parts else ""
            eik = f" eik {parts['eik']:.3e}" if "eik" in parts else ""
            print(f"[Adam] step {step:5d} | total {parts['total']:.4e} | "
                  f"{r_str}init {parts['init']:.3e}{anc}{eik} | "
                  f"lr {parts['lr']:.2e} | {time.time()-t0:.0f}s")

            # 停滞检测：Adam 尾部推进缓慢时提前切到 L-BFGS（同 mech_init.py）
...
def train_lbfgs(phi, phi_ref, train_data: PointSets, val_data: PointSets,
    ...
    for block in range(cfg.lbfgs_blocks):
        ...
        prev_val = val_loss
        # 早停按 early_stop_metric 选择的损失计停滞（回滚仍按 val）：
        # 逐块相对下降 < early_stop_rtol 即计停滞，与力学模块语义一致
...
def save_vn_boundary_figure(mech, phi_ref, cfg: HJBConfig, omega2: float,
    ...
    ys = torch.linspace(0.0, cfg.ly, n_v, dtype=cfg.dtype)
    zero_h, zero_v = torch.zeros_like(xs), torch.zeros_like(ys)
    # 逆时针：下边(左→右) → 右边(下→上) → 上边(右→左) → 左边(上→下)
    segs = [
        ("下边", torch.stack([xs, zero_h], dim=1), cfg.lx),
        ("右边", torch.stack([torch.full_like(ys, cfg.lx), ys], dim=1), cfg.ly),
        ("上边", torch.stack([xs.flip(0), torch.full_like(xs, cfg.ly)], dim=1),
         cfg.lx),
        ("左边", torch.stack([zero_v, ys.flip(0)], dim=1), cfg.ly),
    ]
    s_all, vn_all = [], []
    s0 = 0.0
...
def residual_rms(phi, data: PointSets, cfg: HJBConfig) -> float:
    ...
    _, r, _ = hjb_residual_loss(phi, data.interior, create_graph=False)
...
def main(cfg: Optional[HJBConfig] = None):
    ...
    pools = build_point_sets(cfg, mech, phi_ref, omega2_before, area_before,
                             ...
                             init_corner_n=cfg.pool_init_corner,
                             corner_freeze_n=cfg.pool_corner_freeze)
    ...
    train_data = build_point_sets(cfg, mech, phi_ref, omega2_before, area_before,
                                  ...
                                  init_corner_n=cfg.lbfgs_init_corner,
                                  corner_freeze_n=cfg.lbfgs_corner_freeze)
    val_data = build_point_sets(cfg, mech, phi_ref, omega2_before, area_before,
                                ...
                                init_corner_n=cfg.val_init_corner,
                                corner_freeze_n=cfg.val_corner_freeze)
    print(f"点集：Adam 池 {cfg.pool_interior}+{cfg.pool_init}+"
          f"{cfg.pool_init_band}(带)+{cfg.pool_init_corner}(角) | "
          f"L-BFGS {cfg.lbfgs_interior}+{cfg.lbfgs_init}+"
          f"{cfg.lbfgs_init_band}(带)+{cfg.lbfgs_init_corner}(角) | "
          f"验证 {cfg.val_interior}+{cfg.val_init}+"
          f"{cfg.val_init_band}(带)+{cfg.val_init_corner}(角)"
          + (f" | 角部硬非设计域 {cfg.corner_freeze} 见方（冻结）"
             if cfg.corner_freeze > 0.0 else ""))
