...
变量传递约定（本脚本的核心，改动前务必理解）：
  ...
  5. 种子：HJB 步 seed = seed + k，力学重解 seed = seed + 100000 + k，
     每次迭代点集不同但全程确定可复现。
  6. 罚参数自适应（Step D，alpha_adaptive=True 时）：第 1 步用 alpha 初值，
     之后每步 HJB 结束后按 p = min(τ·ω²/|ΔV|, p_cap)、1/α = p/|ΔV| 更新
     下一步的 α（ω² 取当步 FEM 对拍锚点，ΔV = area_after − C）。
     p_cap 自适应（p_cap_adaptive=True 时）：每步力学重解后用四角探针测
     最弱角应变能，p_cap 跟踪 κ·角部应变能（每步限幅 ±p_cap_rate，硬夹紧
     [p_cap_min, p_cap_max]）——左右边中段 V_n 转负参与演化，
     四角 V_n 恒正不被侵蚀。
...
class LoopConfig:
    ...
    # 循环规模与演化参数
    n_iter: int = 50               # 交替迭代次数，总伪时间 T = n_iter·dt
    dt: float = 0.01               # 每步伪时间步长（自适应罚下 |V_n|≲0.8，|V_n|·dt ≲ β=0.01）
    alpha: float = 1.0             # 罚参数初值（仅第 1 步；alpha_adaptive 开启后每步更新）
    v_target: float = 0.4          # C = 50% × 0.8

    # Step D：罚参数自适应（《方案_详细版》§5.5 公式读法 A + p_cap 保护四角）
    #   罚压力 p = (1/α)·|ΔV| = min(τ·ω²/|ΔV|, p_cap)，ΔV = ∫S dx − C
    #   p_cap 取在左右边中段应变能（≈0.7）与四角应变能（≈1.2）之间：
    #   左右边中段 V_n 转负参与演化，四角 V_n 恒正不被侵蚀。
    #   文档公式的 α_max=1e6 帽子只在 |ΔV|<5e-4 才起作用，无法保护角部，故弃用。
    alpha_adaptive: bool = True
    tau: float = 1.1
    p_cap: float = 1.0

    # p_cap 自适应（角部跟踪 + 安全带）：每步力学重解后测四角应变能密度
    # （四角邻近边段峰值的四角最小值 = 最弱的角），目标值 κ·角部应变能，
    # 每步最多变 ±p_cap_rate，并硬夹紧在 [p_cap_min, p_cap_max]：
    #   下限保证左右边中段（ε:A:ε≈0.4–0.7）持续可侵蚀；上限保护 dt 的
    #   CFL 条件（|V_n|·dt ≲ β）与角部。κ≈0.8 为安全系数。
    p_cap_adaptive: bool = True
    kappa: float = 0.8
    p_cap_min: float = 0.8
    p_cap_max: float = 1.1
    p_cap_rate: float = 0.1            # 每步最大相对变化
    corner_probe_radius: float = 0.05  # 角部探针：距角点此范围内的边段
    corner_probe_n: int = 64           # 每个角每条边段的探针点数
...
def update_alpha(area: float, omega2: float, cfg: LoopConfig,
                 p_cap: float) -> Tuple[float, float]:
    """Step D：罚参数自适应更新（《方案_详细版》§5.5 读法 A + p_cap 保护四角）。

        p = min(τ·ω²/|ΔV|, p_cap)，1/α = p/|ΔV|，ΔV = area − C

    文档公式 1/α = min(ω²τ/(ΔV)², α_max) 中 α_max 只在 |ΔV| < 5e-4 才封顶，
    |ΔV|→0 时罚压力发散、后期会侵蚀四角；改为直接对罚压力 p 封顶。
    p_cap 介于左右边中段应变能（≈0.7）与四角（≈1.2）之间：左右边中段
    V_n = ε:A:ε − p 转负参与演化，四角 V_n 恒正不被侵蚀。
    返回 (alpha, p)，p 即本状态对应的罚压力绝对值。
    """
    dV = abs(area - cfg.v_target)
    p = min(cfg.tau * omega2 / max(dV, 1e-12), p_cap)
    inv_alpha = p / max(dV, 1e-12)
    return 1.0 / inv_alpha, p


def corner_strain_metric(k: int, cfg: LoopConfig) -> float:
    """四角应变能密度探针：四角邻近边段上 ε:A:ε 峰值的四角最小值（最弱的角）。

    用第 k 步的几何（phi_k @ t_k 切片）与力学（*mech_suffix(k)）权重。
    p_cap 必须低于每个角的应变能密度才能保护角部，故取最弱角作度量。
    """
    hcfg = hjb_mod.HJBConfig(lx=cfg.lx, ly=cfg.ly, t_current=k * cfg.dt)
    net_cfg = NetworkConfig(lx=cfg.lx, ly=cfg.ly,
                            hidden_layers=hcfg.mech_hidden_layers,
                            hidden_width=hcfg.mech_hidden_width, dtype=cfg.dtype)
    mech = hjb_mod.build_mechanics_networks(net_cfg)
    for n in MECH_NET_NAMES:
        mech[n].load_state_dict(torch.load(os.path.join(
            cfg.weights_dir, n + mech_suffix(k, cfg)), weights_only=True))
    mech.eval()
    ...
    phi.load_state_dict(torch.load(os.path.join(
        cfg.weights_dir, phi_name(k, cfg)), weights_only=True))
    phi.eval()
    phi.requires_grad_(False)

    r, m = cfg.corner_probe_radius, cfg.corner_probe_n
    s = torch.linspace(0.0, r, m, dtype=cfg.dtype).unsqueeze(1)
    zero, one = torch.zeros_like(s), torch.ones_like(s)
    # 每个角：两条邻边上距角点 r 以内的探针段
    corner_pts = [
        torch.cat([torch.cat([s, zero], 1), torch.cat([zero, s], 1)], 0),          # 左下
        torch.cat([torch.cat([cfg.lx - s, zero], 1),
                   torch.cat([cfg.lx * one, s], 1)], 0),                            # 右下
        torch.cat([torch.cat([s, cfg.ly * one], 1),
                   torch.cat([zero, cfg.ly - s], 1)], 0),                           # 左上
        torch.cat([torch.cat([cfg.lx - s, cfg.ly * one], 1),
                   torch.cat([cfg.lx * one, cfg.ly - s], 1)], 0),                   # 右上
    ]
    metric = math.inf
    for pts in corner_pts:
        with torch.no_grad():
            phi_val = hjb_mod.eval_phi_at(phi, pts, k * cfg.dt)
            _, _, _, e = hjb_mod.fields_from_phi_val(phi_val, pts, hcfg)
        _, _, ex, ey, gxy = hjb_mod.frozen_strain(mech, pts)
        sxx, syy, sxy = hjb_mod.constitutive_stress(ex, ey, gxy, e, hcfg)
        eng = sxx * ex + syy * ey + sxy * gxy
        metric = min(metric, float(eng.max()))
    return metric


def update_p_cap(corner_eng: float, p_cap_old: float, cfg: LoopConfig) -> float:
    """p_cap 自适应：目标值 κ·最弱角应变能，每步限幅 ±p_cap_rate，
    并硬夹紧在 [p_cap_min, p_cap_max]（下限保住左右边参与，上限保住 dt 的 CFL）。"""
    target = cfg.kappa * corner_eng
    if target < cfg.p_cap_min:
        print(f"[警告] 角部应变能 {corner_eng:.3f} 过低（κ·角 = {target:.3f} < "
              f"p_cap_min = {cfg.p_cap_min}），保护窗口正在收窄，建议检查几何")
    lo = (1.0 - cfg.p_cap_rate) * p_cap_old
    hi = (1.0 + cfg.p_cap_rate) * p_cap_old
    new = min(max(target, lo), hi)
    return min(max(new, cfg.p_cap_min), cfg.p_cap_max)
...
def save_history_figure(history: List[Dict], cfg: LoopConfig):
    ...
    # 罚压力 p = |ΔV|/α、p_cap 轨迹与角部应变能监控（同一能量密度量纲）：
    # 安全条件 = p < p_cap < 角部应变能
    ...
    pc = [h.get("p_cap_used") for h in history]
    xs_c = [i for i, v in zip(iters, pc) if v is not None]
    ys_c = [v for v in pc if v is not None]
    if xs_c:
        axes[3].plot(xs_c, ys_c, "s--", color="gray", ms=4, label="p_cap")
    ce = [h.get("corner_eng_min") for h in history]
    xs_e = [i for i, v in zip(iters, ce) if v is not None]
    ys_e = [v for v in ce if v is not None]
    if xs_e:
        axes[3].plot(xs_e, ys_e, "^-", color="tab:red", ms=4,
                     label="corner ε:A:ε (min)")
    axes[3].set_title("penalty p / p_cap / corner energy")
...
def main(cfg: Optional[LoopConfig] = None):
    ...
    if cfg.alpha_adaptive:
        print(f"罚参数：Step D 自适应（第 1 步 α = {cfg.alpha}，之后 "
              f"p = min(τ·ω²/|ΔV|, p_cap)，τ = {cfg.tau}）")
        if cfg.p_cap_adaptive:
            print(f"p_cap 自适应：初值 {cfg.p_cap}，目标 κ·角部应变能（κ = {cfg.kappa}），"
                  f"每步限幅 ±{cfg.p_cap_rate:.0%}，夹紧 [{cfg.p_cap_min}, {cfg.p_cap_max}]")
        else:
            print(f"p_cap 固定 = {cfg.p_cap}")
    else:
    ...
    # 罚参数：第 1 步用初值 cfg.alpha，之后每步由 Step D 按上步面积与 ω² 锚点更新
    alpha_cur = cfg.alpha
    p_cap_cur = cfg.p_cap
    ...
    for k in range(1, cfg.n_iter + 1):
        ...
        p_cap_used = p_cap_cur
        print("\n" + "#" * 72)
        print(f"### 迭代 {k}/{cfg.n_iter}（{mode}）：t ∈ [{(k - 1) * cfg.dt}, {k * cfg.dt}]"
              f" | α = {alpha_used:.4g} | p_cap = {p_cap_used:.3f}")
        print("#" * 72)
        ...
        if cfg.alpha_adaptive and state.get("area_after") is not None:
            alpha_cur, p_next = update_alpha(state["area_after"], omega2_anchor,
                                             cfg, p_cap_cur)
            print(f"[Step D] ΔV = {state['area_after'] - cfg.v_target:+.4f}，"
                  f"ω² = {omega2_anchor:.4f} → 下一步 α = {alpha_cur:.4g}"
                  f"（罚压力 p = {p_next:.4f}，p_cap = {p_cap_cur:.3f}）")
        ...
        # 角部探针 + p_cap 自适应（供下一步 Step D 使用；力学跳步时权重已复制，
        # 探针仍在新几何 phi_k 上评估，度量保持有效）
        corner_eng = None
        if cfg.p_cap_adaptive:
            corner_eng = corner_strain_metric(k, cfg)
            p_cap_new = update_p_cap(corner_eng, p_cap_cur, cfg)
            print(f"[p_cap] 最弱角应变能 = {corner_eng:.3f}（κ·角 = "
                  f"{cfg.kappa * corner_eng:.3f}）→ 下一步 p_cap = {p_cap_new:.3f}")
            p_cap_cur = p_cap_new
        ...
        record = {
            ...
            "p_cap_used": p_cap_used,
            "corner_eng_min": corner_eng,
            "max_dphi": dphi,
