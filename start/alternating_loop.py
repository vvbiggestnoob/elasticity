...
class LoopConfig:
    ...
    # 循环规模与演化参数
    n_iter: int = 10               # 交替迭代次数，总伪时间 T = n_iter·dt
    dt: float = 0.05                # 每步伪时间步长（与 hjb 模块当前默认一致）
...
def run_hjb_step(k: int, cfg: LoopConfig, cheap: bool) -> Dict:
    ...
    kwargs = dict(
        lx=cfg.lx, ly=cfg.ly,
        t_current=t0, dt=cfg.dt, alpha=cfg.alpha, v_target=cfg.v_target,
...
def main(cfg: Optional[LoopConfig] = None):
    ...
    print(f"交替循环：n_iter = {cfg.n_iter}，dt = {cfg.dt}，"
          f"总伪时间 T = {cfg.n_iter * cfg.dt}，α = {cfg.alpha}，C = {cfg.v_target}")
    ...
    for k in range(1, cfg.n_iter + 1):
        ...
        state = run_hjb_step(k, cfg, cheap)
